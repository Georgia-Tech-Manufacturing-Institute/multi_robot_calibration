...
H_W1V1_vision = invert(pcregistericp(W1_cloud,V1_cloud));
H_W2V1_vision = invert(pcregistericp(W2_cloud,V1_cloud));

%% Plot Stuff!
H1_vision = double(H_W1V1_vision.A);
H2_vision = double(H_W2V1_vision.A);

figure(2)
hold on
ax1 = plotTransforms(se3(eye(4,4)),'FrameAxisLabels',"on","FrameLabel","Origin","FrameSize",3);
ax2 = plotTransforms(se3(H1_vision),'FrameAxisLabels',"on","FrameLabel","W1","FrameSize",3);
ax3 = plotTransforms(se3(H2_vision),'FrameAxisLabels',"on","FrameLabel","W2","FrameSize",3);
axis padded
axis equal
grid on
view(160, 30)
